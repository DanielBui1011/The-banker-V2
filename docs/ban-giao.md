...
| 24 | Giai đoạn 3 viết lại trong trang Ứng vốn: A2 → chọn bên nhận (không tích sẵn, ≥1) → 3 chào giá (số căn phải, nhãn chữ "Lãi thấp nhất", tiền lãi tính theo giá trị từng chào giá: C 80 triệu ≈ 171 nghìn, không còn 182) → ký A4 trên trang bên được chọn (Techcombank: khung Techcombank; B/C: khung trung tính, màn chuyển tiếp trung tính) → ghi sổ khóa thật → chứng thư. Techcombank đi tiếp như hành trình 1; B/C dừng ở thẻ kết + [Chọn lại chào giá] [Bắt đầu lại]; C khóa 44 + 36 (`lockPlan` trong journey.js). Gỡ `LegacyScreen`, `Screen10`, `src/screens/`, `resetSignal`, CSS `[data-legacy-screen]` |
...
**Vòng 24:** đã gỡ hết — `LegacyScreen`, `Screen10`, thư mục `src/screens/`, `resetSignal` (appState) và CSS `[data-legacy-screen] footer`. Không còn adapter tạm nào. Phần dưới là lịch sử.
