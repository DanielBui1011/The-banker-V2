...
const DARK_KEY_HINT = 'border-slate-600 bg-slate-800 text-slate-50'
...
export default function ScenarioPanel({ onReset }) {
  ...
  return (
    <>
      {activeLabel && (
        <div className="fixed top-20 right-6 z-[70] rounded-full border border-slate-600 bg-slate-900/90 px-4 py-1.5 text-label font-semibold text-slate-50 shadow-lg">
          {activeLabel}
        </div>
      )}
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-[80] flex h-12 w-12 items-center justify-center rounded-full border border-slate-700 bg-slate-900 text-emphasis text-slate-50 shadow-lg transition hover:bg-slate-800"
      >
        ⚙
      </button>
      ...
      {panelOpen && (
        <div className="fixed bottom-24 right-6 z-[80] w-80 rounded-xl border border-slate-700 bg-slate-900 p-5 shadow-xl">
          <div className="mb-4 flex items-center justify-between">
            <div className="text-emphasis font-semibold text-slate-50">Bảng điều khiển kịch bản</div>
            <button onClick={closePanel} className="text-body text-slate-50 hover:text-slate-50">
              ✕
            </button>
          </div>
          ...
          <div className="mt-4 rounded-lg border border-slate-700 bg-slate-800/50 px-4 py-3 text-body text-slate-50">
            <div className="font-semibold text-slate-50 mb-1">Phím tắt khác</div>
          ...
          <button
            onClick={onReset}
            className="mt-3 w-full rounded-lg border border-slate-600 py-2 text-body font-semibold text-slate-50 transition hover:bg-slate-800"
          >
            Đặt lại (phím R)
          </button>
        </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2.5">
      <div className="flex items-center gap-2">
        <div className={`text-body font-medium text-slate-50`}>{label}</div>
        <KeyHint label={hintKey} className={DARK_KEY_HINT} />
      </div>
      {locked ? (
        <span className="text-emphasis text-slate-50" aria-label="Đã khóa">
          🔒
        </span>
      ) : (
        <button
